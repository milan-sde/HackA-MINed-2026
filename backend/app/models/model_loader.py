...
# ---------------------------------------------------------------------------
# Resolve paths for both refactored backend layout and legacy root layout
# ---------------------------------------------------------------------------
_THIS_DIR    = os.path.dirname(os.path.abspath(__file__))         # backend/app/models
...
_MODEL_DIR_CANDIDATES = [
    ...
    os.path.join(_ROOT_DIR, "models"),
]
_RAW_DIR_CANDIDATES = [
    os.path.join(_BACKEND_DIR, "data", "raw"),
    os.path.join(_ROOT_DIR, "data", "raw"),
]

def _has_required_models(path: str) -> bool:
    return os.path.exists(os.path.join(path, "xgboost_model.pkl"))


MODELS_DIR = next(
    (p for p in _MODEL_DIR_CANDIDATES if os.path.isdir(p) and _has_required_models(p)),
    next((p for p in _MODEL_DIR_CANDIDATES if os.path.isdir(p)), _MODEL_DIR_CANDIDATES[0]),
)
RAW_DATA_DIR = next((p for p in _RAW_DIR_CANDIDATES if os.path.isdir(p)), _RAW_DIR_CANDIDATES[0])
